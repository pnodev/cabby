...
export async function getCachePath(): Promise<string> {
  ...
  const cachePath = process.env.FILE_CACHE_PATH || path.join(storagePath, '.cache')
...
export function parseSize(sizeParam: string | undefined): { width?: number; height?: number } | undefined {
...
export async function getOutputFormat(formatParam: string | undefined, originalPath: string): Promise<string> {
  if (formatParam) {
...
// Get cache file path
export async function getCacheFilePath(filePath: string, cacheKey: string): Promise<string> {
  const path = await getPath()
...
export const transformImage = createServerFn().inputValidator(TransformImageInputSchema).handler(async (
  ctx
): Promise<void> => {
  ...
  if (!isSupportedImageFormat(format)) {
    throw new Error(`Unsupported image format: ${format}. Supported formats: webp, png, jpg, jpeg`)
  }
...
export const getFile = createServerFn().inputValidator(GetFileInputSchema).handler(async (ctx): Promise<{ buffer: Buffer; contentType: string; cacheKey?: string }> => {
...
export const getAllFiles = createServerFn().handler(async (): Promise<Array<{ path: string; isImage: boolean; hasCache: boolean; cacheCount: number }>> => {
  const path = await getPath()
  const fs = await getFs()
  const storagePath = getStoragePath()
  const cacheDir = await getCachePath()
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const cacheCount = isImage ? await countCacheFiles(relativePath, cacheDir) : 0
          files.push({
            path: relativePath,
            isImage,
...
async function countCacheFiles(imagePath: string, cacheDir: string): Promise<number> {
  try {
    const path = await getPath()
    const fs = await getFs()
...
export const getCachedVersions = createServerFn().inputValidator(GetCachedVersionsInputSchema).handler(async (ctx): Promise<Array<{
  fileName: string
  size?: string
  format: string
  url: string
